		...
		if #role == "follower" {
			service: "\(#id)-jormungandr-rest": {
				...
				tags: [
					...
					"traefik.http.routers.\(namespace)-jormungandr-rpc.middlewares=rpc-ratelimit@consulcatalog, remove-origin@consulcatalog, cors-headers@consulcatalog",
					"traefik.http.middlewares.remove-origin.headers.customrequestheaders.Origin=http://127.0.0.1",
					"traefik.http.middlewares.cors-headers.headers.accesscontrolallowmethods=GET,OPTIONS,PUT,POST",
